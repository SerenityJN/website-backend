...
/* ===========================================================
   🎓 ENROLLMENT ROUTE - COMPLETE FIXED VERSION
   =========================================================== */
router.post("/enroll", upload, async (req, res) => {
  ...
  try {
    ...
    // Handle Returnee Students (Add your returnee code here)
    else if (student_type === "Returnee") {
      // TODO: Add your returnee logic here
      // Remember to set studentLRN and studentLastName if needed
      await conn.rollback();
      conn.release();
      return res.status(501).json({ 
        success: false, 
        message: "Returnee enrollment is not yet implemented." 
      });
    }
    else {
      await conn.rollback();
    ...
    try {
      ...
      const studentLastName = studentLastName || req.body.lastname || "";
      ...
      // Don't rollback, enrollment was successful
    }
